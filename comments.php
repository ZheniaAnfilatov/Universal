...
//  создаем свою функцию каждого комментария
  function universal_theme_comment( $comment, $args, $depth ) {
    // проверям в каком стиле у нас родитель (ol ul или div)
	if ( 'div' === $args['style'] ) {
    // если стиль div, то тег будет div
		$tag       = 'div';
		$add_below = 'comment';
	} else {
    // в другом случае будет li
		$tag       = 'li';
      ...
      <ol class="comments-list">
        ...
        wp_list_comments(
          array(
            ...
            'login_text' => 'Зарегистрируйтесь, чтобы оставлять комментарии'
          )
        );
        ?>
      </ol><!-- .comment-list -->
